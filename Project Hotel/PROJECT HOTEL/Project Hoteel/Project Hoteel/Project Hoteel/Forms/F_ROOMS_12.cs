...
namespace Project_Hoteel
{
    public partial class F_ROOMS_12 : Form
    {
        ...
        string connstr111 = "Data Source="+C_LOGIN_2.SERVER1+"; Initial Catalog=Hoteel Reservation;Integrated Security = True";
        ...
        private void b_add_room_12_Click(object sender, EventArgs e)
        {
            try
            {
                using (SqlConnection sqlconn = new SqlConnection(connstr))
                {
                    ...
                    sqlcmd.Parameters.AddWithValue("@roomNumber", t_number_room_12.Text);
                    sqlconn.Open();
                    ...
                    using (SqlDataReader dread = sqlcmd.ExecuteReader())
                    {
                        ...
                        {
                            if (cbox_type_room_12.Text != "" && t_price_room_12.Text != "")
                            {
                                InsertRoom();
                            }
                            else
        ...
        private void InsertRoom()
        {
            try
            {
                using (SqlConnection sqlconnInsert = new SqlConnection(connstr))
                {
                    SqlCommand sqlcmdInsert = new SqlCommand("InsertRoomData", sqlconnInsert);
                    sqlcmdInsert.CommandType = CommandType.StoredProcedure;

                    sqlcmdInsert.Parameters.AddWithValue("@roomNumber", int.Parse(t_number_room_12.Text));
                    sqlcmdInsert.Parameters.AddWithValue("@roomType", cbox_type_room_12.SelectedItem.ToString());
                    sqlcmdInsert.Parameters.AddWithValue("@roomCondition", "فارغة");
                    sqlcmdInsert.Parameters.AddWithValue("@roomPrice", float.Parse(t_price_room_12.Text));

                    sqlconnInsert.Open();
                    sqlcmdInsert.ExecuteNonQuery();

                    l_notificatio_12.Text = "تمت عملية الإضافة بنجاح";
        ...
        private void LoadRoom()
        {
            ...
            try
            {
                sqlconn.ConnectionString = connstr111;
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message);
